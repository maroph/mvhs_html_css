...
Eyeball.prototype = {
    ...
    follow: function (x1, y1) {
        ...
        if (Math.abs(x1 - this.startX) > this.max || Math.abs(y1 - this.startY) > this.max) {
            ...
            let angle = Math.atan2(y1 - (this.startY + this.canvasY), x1 - (this.startX + this.canvasX));

            let movX = Math.cos(angle) * this.max;
            let movY = Math.sin(angle) * this.max;
...
function Guy(options) {
    ...
    if (options.id) {
        let node = document.getElementById(options.id);
        if (node) {
    ...
    let g = this.domElement.getContext('2d'),
        that = this;


    let head = new Head(60 * scale, 60 * scale, 50 * scale, this.color),
    ...
    let canvasPos = findPos(this.domElement);
